...
router.get('/google/callback', async (req: Request, res: Response) => {
  try {
    ...
    // Redirect to admin with access token in URL (frontend reads it once and stores it)
    res.redirect(`${process.env.FRONTEND_URL}/auth/callback?token=${encodeURIComponent(accessToken)}&name=${encodeURIComponent(user.name)}&role=${user.role}`);
  } catch (err) {
